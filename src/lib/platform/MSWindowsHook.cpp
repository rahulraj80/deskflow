...
static LRESULT CALLBACK keyboardLLHook(int code, WPARAM wParam, LPARAM lParam)
{
  if (code >= 0) {
    ...
    if (g_isPrimary && injected && !g_forwardTouchscreenEvents) {
      LOG_DEBUG(
          "hook: keyboard injected event filtered (touch forwarding disabled) vk=0x%02x scan=%u flags=0x%08x",
    ...
    if (g_isPrimary && injected) {
      LOG_DEBUG(
          "hook: keyboard injected event forwarded (touch forwarding enabled) vk=0x%02x scan=%u", info->vkCode,
          info->scanCode
      );
    }
...
static LRESULT CALLBACK mouseLLHook(int code, WPARAM wParam, LPARAM lParam)
{
  if (code >= 0) {
    ...
    if (!g_isPrimary && injected) {
      LOG_DEBUG(
          "hook: mouse injected event passed through (non-primary screen) msg=0x%04x x=%d y=%d", wParam, info->pt.x,
          info->pt.y
      );
    ...
    if (g_isPrimary && injected && !g_forwardTouchscreenEvents) {
      LOG_DEBUG(
          "hook: mouse injected event filtered (touch forwarding disabled) msg=0x%04x x=%d y=%d flags=0x%08x", wParam,
    ...
    if (g_isPrimary && injected) {
      LOG_DEBUG(
          "hook: mouse injected event forwarded (touch forwarding enabled) msg=0x%04x x=%d y=%d", wParam, info->pt.x,
          info->pt.y
      );
    }
